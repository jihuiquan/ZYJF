...
public class SetJiaoYiMimaActivity extends BaseActivity implements View.OnClickListener,TextWatcher {
    ...
    private void initView() {
        ...
        mIvIcon = (ImageView) findViewById(R.id.activity_setjiaoyimima_icon);
    }
        ...
        switch (v.getId()){
            case R.id.bt_sure:
                if (mEt_newkey.getText().toString().length() != 6){
                    ...
                    showLoadingDialog();
                    if (null != mAction && mAction.equals("reSetPW")){
                        new HttpHelper().resetBuyPassword(mEt_newkey.getText().toString()).subscribe(new Action1<SetBuyPasswordResponse>() {
                            @Override
                            public void call(SetBuyPasswordResponse setBuyPasswordResponse) {
                                ...
                                cacheData.result.userPaypassword = mEt_newkey.getText().toString();
                                DataCache.instance.saveCacheData("heng", "MyUserInfo",cacheData);
                                ViewHelper.showToast(SetJiaoYiMimaActivity.this, "重置交易密码成功");
                                setResult(1002);
                                finish();
                            }
                        ...
                        new HttpHelper().setBuyPassword(mEt_newkey.getText().toString()).subscribe(new Action1<SetBuyPasswordResponse>() {
                            @Override
                            public void call(SetBuyPasswordResponse setBuyPasswordResponse) {
                                dismissLoadingDialog();
                                MyUserInfo cacheData = DataCache.instance.getCacheData("heng", "MyUserInfo");
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        if (mEt_newkey.getText().toString().length()>0){
            ...
            mBt_sure.setBackgroundResource(R.drawable.bg_submit_round_rect);
