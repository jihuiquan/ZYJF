...
public class MyPopupWindow implements View.OnClickListener {
    ...
    public MyPopupWindow(Activity context, List<WelfareModel> list, String name, String money, int dayLimit) {
        mContext = context;
        mList = new ArrayList<>();
        mList = list;
        mname = name;
        this.money = money;
        this.investDay = dayLimit;
        initPopupWindow();//初始化弹窗
    }

    private void initPopupWindow() {
        View view = LayoutInflater.from(mContext).inflate(R.layout.popwindow_youhuiquan, null);
        lv_youhui = (ListView) view.findViewById(R.id.youhuiquan_list);
        pop_close = (TextView) view.findViewById(R.id.pop_close);
        TextView youhui_name = (TextView) view.findViewById(R.id.youhui_name);
        youhui_name.setText(mname);
        mPopWindow = new PopupWindow(view, RelativeLayout.LayoutParams.MATCH_PARENT, (int) (DeviceUtils.deviceHeight(mContext)/1.80), true);
        ...
        mainGetPopCouAdapter.setInvestLimit(investDay);
        ...
        lv_youhui.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
                ...
                if (mList.get(position).isAvailable()) {
                    ...
                    WelfareModel bean = mList.get(position);
                    if (bean.isSelect) {
                        bean.isSelect = false;
